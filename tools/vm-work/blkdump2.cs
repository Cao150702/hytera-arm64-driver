...
public class BlkDump2 {
    ...
    static byte[] RdBlock(int addr, ushort prefix) {
        int size = 0x5DC;
        ...
        Wr(Frame(0x01, 0, prefix, Hrcp(0x01C7, 12, 0, data)));
        rlen = 0;
        return ReadResponse(8.0);
    }
    public static void Main() {
        ...
        for (int i = 0; i < init.Length; i++) {
            ...
            Log("init" + (i+1) + " resp=" + (fr != null ? fr.Length.ToString() : "0"));
        }
        // LONG wait for the late exchange response
        DateTime t1 = DateTime.Now;
        while ((DateTime.Now - t1).TotalSeconds < 15.0) {
            byte[] fr = GetFrame();
            if (fr != null && fr[1] == 0x04) {
                Log("LATE resp len=" + fr.Length + " hex=" + BitConverter.ToString(fr).Replace("-",""));
            }
            Poll(250);
        }
        Log("late-wait done");
        byte[] r4 = RdBlock(6000, 0x3bbb);
        Log("block4 len=" + (r4 != null ? r4.Length : 0) + (r4 != null ? " head=" + BitConverter.ToString(r4).Replace("-","").Substring(0, 120) : ""));
        byte[] r5 = RdBlock(7500, 0x36c1);
        Log("block5 len=" + (r5 != null ? r5.Length : 0) + (r5 != null ? " head=" + BitConverter.ToString(r5).Replace("-","").Substring(0, 120) : ""));
        Log("DONE");
